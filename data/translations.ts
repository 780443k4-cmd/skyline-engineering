...
const pages = {
  uk: {
    ...
    locationPages: {
      ...
      benidorm: {
        ...
        title: 'Будівництво вілл під ключ\nу Бенідормі.',
        intro: 'SKYLINE Engineering — будівельна компанія, що спеціалізується на будівництві приватних вілл під ключ у Бенідормі, Фінестраті та на Коста-Бланці. Ми будуємо вілли як у самому Бенідормі, так і в навколишніх урбанізаціях, працюючи з нашої бази за кілька хвилин їзди — у Фінестраті.',
        bodyTitle: 'Чому Бенідорм',
        bodyText: 'Бенідорм поєднує цілорічний попит на оренду, велику міжнародну спільноту мешканців і розвинену інфраструктуру — саме тому значна частина запитів на будівництво вілли надходить саме щодо Бенідорма. Більшість ділянок під вілли розташовані у сформованих урбанізаціях із наявними комунікаціями (вода, електрика, під’їзні шляхи), що скорочує технічний етап підключення порівняно з більш віддаленими ділянками. При будівництві вілли в Бенідормі ми супроводжуємо весь цикл: ділянка та її перевірка → архітектура і дизайн → проєкт → дозвіл на будівництво (ліцензія) від мерії Бенідорма → будівництво → інженерні системи → оздоблення → басейн і благоустрій → здача об’єкта.',
        ...
        faq: [
          ...
          { q: 'Як обрати будівельну компанію для будівництва вілли в Бенідормі?', a: 'Звертайте увагу на реальний портфель уже зданих об’єктів із фотографіями, єдину відповідальність за весь цикл — від ділянки й ліцензії до здачі під ключ — і локальну присутність у регіоні. SKYLINE Engineering зареєстрована та базується у сусідньому Фінестраті, координує дозволи з меріями Бенідорма і Фінестрата та публікує реальні завершені об’єкти в розділі «Проєкти».' },
          { q: 'Чи можна замовити будівництво вілли під ключ у Бенідормі?', a: 'Так — SKYLINE Engineering супроводжує весь цикл: ділянка та її перевірка, архітектура і дизайн, дозвіл на будівництво, будівництво, інженерні системи, оздоблення, басейн і благоустрій, здача об’єкта. Це єдиний скоординований процес з однією контактною особою, а не набір розрізнених підрядників.' },
          { q: 'Чи може SKYLINE Engineering вести проєкт від ділянки й ліцензії до готового будинку?', a: 'Так, це наша стандартна модель роботи: одна компанія відповідає за архітектуру, інженерію, отримання дозволу на будівництво та саме будівництво вілли в Бенідормі — від первинного аналізу ділянки до передачі ключів.' },
          { q: 'Чи можна контролювати будівництво вілли в Бенідормі, перебуваючи за межами Іспанії?', a: 'Так — у клієнта є одна контактна особа в команді, на зв’язку через WhatsApp, Telegram, Viber, телефон та email незалежно від того, де ви перебуваєте. Реалізовані та ті, що будуються, об’єкти також фіксуються в розділі «Проєкти» сайту з реальними фотографіями.' },
        ],
    ...
    projectsPage: {
      ...
      recentProjectEyebrow: 'Реальний завершений об’єкт',
  ...
  ru: {
    ...
    home: {
      ...
      collectionTitle: 'Концепции вилл для Бенидорма',
      collectionText:
    ...
    locationPages: {
      ...
      benidorm: {
        ...
        title: 'Строительство вилл под ключ\nв Бенидорме.',
        intro: 'SKYLINE Engineering — строительная компания, специализирующаяся на строительстве частных вилл под ключ в Бенидорме, Финестрате и на Коста-Бланке. Мы строим виллы как в самом Бенидорме, так и в окрестных урбанизациях, работая с нашей базы в нескольких минутах езды — в Финестрате.',
        bodyTitle: 'Почему Бенидорм',
        bodyText: 'Бенидорм сочетает круглогодичный спрос на аренду, крупное международное сообщество жителей и развитую инфраструктуру — поэтому значительная часть запросов на строительство виллы касается именно Бенидорма. Большинство участков под виллы расположены в сформированных урбанизациях с готовыми коммуникациями (вода, электричество, подъездные пути), что сокращает технический этап подключения по сравнению с более удалёнными участками. При строительстве виллы в Бенидорме мы сопровождаем весь цикл: участок и его проверка → архитектура и дизайн → проект → разрешение на строительство (лицензия) от мэрии Бенидорма → строительство → инженерные системы → отделка → бассейн и благоустройство → сдача объекта.',
        ...
        faq: [
          ...
          { q: 'Как выбрать строительную компанию для строительства виллы в Бенидорме?', a: 'Обращайте внимание на реальный портфель уже сданных объектов с фотографиями, единую ответственность за весь цикл — от участка и лицензии до сдачи под ключ — и локальное присутствие в регионе. SKYLINE Engineering зарегистрирована и базируется в соседнем Финестрате, координирует разрешения с мэриями Бенидорма и Финестрата и публикует реальные завершённые объекты в разделе «Проекты».' },
          { q: 'Можно ли заказать строительство виллы под ключ в Бенидорме?', a: 'Да — SKYLINE Engineering сопровождает весь цикл: участок и его проверка, архитектура и дизайн, разрешение на строительство, строительство, инженерные системы, отделка, бассейн и благоустройство, сдача объекта. Это единый скоординированный процесс с одним контактным лицом, а не набор разрозненных подрядчиков.' },
          { q: 'Может ли SKYLINE Engineering вести проект от участка и лицензии до готового дома?', a: 'Да, это наша стандартная модель работы: одна компания отвечает за архитектуру, инженерию, получение разрешения на строительство и само строительство виллы в Бенидорме — от первичного анализа участка до передачи ключей.' },
          { q: 'Можно ли контролировать строительство виллы в Бенидорме, находясь за пределами Испании?', a: 'Да — у клиента есть одно контактное лицо в команде, на связи по WhatsApp, Telegram, Viber, телефону и email независимо от того, где вы находитесь. Реализованные и строящиеся объекты также фиксируются в разделе «Проекты» сайта с реальными фотографиями.' },
        ],
    ...
    projectsPage: {
      ...
      recentProjectEyebrow: 'Реальный завершённый объект',
  ...
  en: {
    ...
    projectsPage: {
      ...
      recentProjectEyebrow: 'Real completed project',
  ...
  es: {
    ...
    projectsPage: {
      ...
      recentProjectEyebrow: 'Proyecto real finalizado',
